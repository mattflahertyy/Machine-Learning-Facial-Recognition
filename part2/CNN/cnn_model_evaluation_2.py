...
test_data = pd.read_csv('../csv_split_data/csv_fixed_label/test_data.csv')
...
class BestVariantCNN(nn.Module):
    ...
    def calculate_fc_input_size(self):
        with torch.no_grad():
            dummy_input = torch.randn(1, 3, 224, 224)  # Adjust the input size if necessary
...
# Load models
main_model = load_model('./training_results/7_final_model.pth', BestVariantCNN)
variant1_model = load_model('./training_results/8_final_variant1.pth', Variant1CNN)
variant2_model = load_model('./training_results/9_final_variant2.pth', Variant2CNN)
