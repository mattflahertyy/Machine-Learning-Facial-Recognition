...
train_dataset = FacialDataset(csv_file='../csv/post_bias/train_augmented.csv', transform=transform)
val_dataset = FacialDataset(csv_file='../csv/post_bias/validation_augmented.csv', transform=transform)
test_dataset = FacialDataset(csv_file='../csv/post_bias/test_augmented.csv', transform=transform)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=20, patience=3):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_loss:
            ...
            best_model_wts = model.state_dict()
            patience_counter = 0
            torch.save(model.state_dict(), 'main_model_augmented.pth')
        else:
            patience_counter += 1
